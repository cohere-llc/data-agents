...
class TestREADMEExamples:
    ...
    def test_imports_available(self):
        ...
        from data_agents import RESTAdapter, Router, TabularAdapter
    ...
    def test_multiple_data_sources_example(self):
        ...
        products_df = pd.DataFrame(
            {"product_id": [1, 2, 3], "name": ["Widget A", "Widget B", "Widget C"]}
        )
    ...
    def test_configuration_example_structure(self):
        ...
        config_example = {
            "adapters": {
                ...
                "my_data": {"type": "tabular", "csv_file": "path/to/data.csv"},
            }
        }
    ...
    def test_python_code_blocks_syntax(self):
        ...
        for i, code_block in enumerate(code_blocks):
            try:
                ...
                pytest.fail(
                    f"Syntax error in README Python code block {i}: {e}\n"
                    f"Code:\n{code_block}"
                )
        ...
        for i, bash_block in enumerate(bash_blocks):
            ...
            for line_num, line in enumerate(lines):
                ...
                if "data-agents" in line and not line.startswith("uv run"):
                    ...
                    if not any(
                        skip in line for skip in ["git clone", "cd ", "pip install"]
                    ):
                        pytest.fail(
                            f"README bash block {i}, line {line_num + 1}: "
                            f"CLI commands should use 'uv run data-agents' "
                            f"not bare 'data-agents'\n"
                            f"Found: {line}"
                        )
    ...
    def test_imports_in_readme_are_available(self):
        ...
        for code_block in code_blocks:
            for line in code_block.split("\n"):
                line = line.strip()
                if line.startswith("from data_agents import") or line.startswith(
                    "import data_agents"
                ):
...
class TestREADMEConsistency:
    ...
    def test_available_adapters_documented(self):
        ...
        from data_agents import RESTAdapter, TabularAdapter
