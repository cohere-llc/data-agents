...
def get_parameter_codes(
    ...
    if param_type:
        filtered_params = {
            code: info for code, info in params.items() if info["type"] == param_type
        }
    else:
...
def main():
    parser = argparse.ArgumentParser(
        description="Get parameter codes for NASA POWER API queries"
    )
    ...
    parser.add_argument(
        "--type", help="Parameter type filter (METEOROLOGY, RADIATION, etc.)"
    )
    parser.add_argument(
        "--max", type=int, default=20, help="Maximum number of parameters"
    )
    parser.add_argument(
        "--format",
        choices=["comma", "list"],
    ...
    codes = get_parameter_codes(
        args.json_file, args.community, args.temporal, args.type, args.max
    )
