...
def test_nasa_adapter_regional_query():
    ...
    try:
        ...
        print(
            f"Regional query result shape: "
            f"{result.shape if hasattr(result, 'shape') else 'No shape'}"
        )
...
def test_nasa_adapter_validation_errors():
    ...
    with pytest.raises(
        ValueError, match="Point queries require latitude and longitude"
    ):
        adapter.query(
            "T2M",
            temporal="daily",
...
def test_nasa_adapter_query_error_handling():
    ...
    with patch.object(requests.Session, "get") as mock_get:
        ...
        with pytest.raises(
            requests.RequestException, match="NASA POWER API request failed"
        ):
            adapter.query(
                "T2M",
                temporal="daily",
...
def test_nasa_adapter_response_parsing():
    ...
    features_data = {
        "features": [
            {
                "properties": {
                    "parameter": {"T2M": {"20240101": 15.5, "20240102": 16.2}}
                },
    ...
    properties_data = {
        "properties": {"parameter": {"T2M": {"20240101": 15.5}}},
        "geometry": {"coordinates": [-74.0, 40.7]},
    }
    ...
    alt_data = {
        "properties": {"some_other_data": "test"},
        "geometry": {"coordinates": [-74.0, 40.7]},
    }
...
def test_nasa_adapter_parameter_superset_edge_cases():
    ...
    with patch.object(adapter, "_fetch_all_parameters") as mock_fetch:
        ...
        assert (
            missing_name_param["name"] == "MISSING_NAME"
        )  # Should use param code as fallback
...
def test_nasa_adapter_discovery_edge_cases():
    ...
    with patch.object(adapter, "_get_parameter_superset") as mock_superset:
        mock_superset.return_value = {
            ...
            "EMPTY_NAME": {"name": None, "definition": None, "units": None},
        }
        ...
        assert (
            "EMPTY_NAME" in empty_param["description"]
        )  # Should use param code as fallback
