...
def find_parameters_by_type(data: Dict[str, Any], param_type: str) -> List[Dict[str, Any]]:
...
def get_parameter_list_for_query(data: Dict[str, Any], community: str, temporal: str, 
                                 param_types: List[str] = None) -> List[str]:
    """
    ...
    if param_types:
        ...
        return [
            param_code for param_code, param_info in params.items()
...
def main():
    ...
    json_files = [
        "ag_daily_sample.json",
        "all_daily_parameters.json"
    ]
    ...
    for json_file in json_files:
        ...
        print(f"Temporal frequencies: {', '.join(data['metadata']['fetched_temporal_frequencies'])}")
        
        if 'summary' in data:
            print(f"Total unique parameters: {data['summary']['total_unique_parameters']}")
        ...
        if 'AG' in data['parameters'] and 'daily' in data['parameters']['AG']:
            meteor_codes = get_parameter_list_for_query(data, 'AG', 'daily', ['METEOROLOGY'])
