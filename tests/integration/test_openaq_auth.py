...
class TestOpenAQAuthentication:
    ...
    def test_openaq_v3_real_api_call(self):
        ...
        try:
            ...
            assert "id" in result.columns or len(result.columns) > 0, "Response should have data columns"
            
            print(f"✓ OpenAQ v3 API test successful: {len(result)} records retrieved")
            print(f"  Response columns: {list(result.columns)[:5]}")  # Show first 5 columns
    ...
    def test_openaq_v3_missing_token_error(self):
        ...
        with pytest.raises(ValueError, match="Environment variable 'NONEXISTENT_OPENAQ_KEY' not found"):
