...
A specialized adapter for the NASA POWER API that provides a
parameter-centric interface. Each parameter is exposed as a discoverable
record type, and queries are made by parameter name with temporal and
community specified as kwargs.
"""
...
class NasaPowerAdapter(Adapter):
    ...
    def _fetch_all_parameters(self) -> dict[str, Any]:
        ...
        Returns:
            Dictionary containing all parameters organized by community
            and temporal frequency
        """
        if self._parameter_cache is not None:
            return self._parameter_cache

        all_parameters: dict[str, dict[str, dict[str, Any]]] = {}
        ...
        for community in self.COMMUNITIES:
            ...
            for temporal in self.TEMPORAL_ENDPOINTS.keys():
                try:
                    ...
                    print(
                        f"Warning: Failed to fetch {community}/{temporal} "
                        f"parameters: {e}"
                    )
    ...
    def _get_parameter_superset(self) -> dict[str, Any]:
        """
        Get the superset of all unique parameters across all communities
                ...
                for param_code, param_info in params.items():
                    ...
                    if param_code not in superset:
                        ...
                        superset[param_code] = {
                            ...
                            "definition": param_info.get(
                                "definition", f"Parameter {param_code}"
                            ),
                    ...
                    if param_code in superset:
                        availability = {"community": community, "temporal": temporal}
    ...
    def discover(self) -> dict[str, Any]:
        ...
        for param_code, param_info in parameter_superset.items():
            ...
            if not isinstance(param_info, dict):
                print(
                    f"Warning: param_info is not a dict for {param_code}: "
                    f"{type(param_info)}"
                )
                continue
            ...
            name = (
                param_info.get("name", param_code) or param_code
            )  # Use param_code as fallback
            definition = (
                param_info.get("definition", f"Parameter {param_code}")
                or f"Parameter {param_code}"
            )  # Use fallback
    ...
    def _build_query_url(self, temporal: str, spatial_type: str) -> str:
        ...
        if temporal not in self.TEMPORAL_ENDPOINTS:
            ...
            raise ValueError(
                f"Invalid temporal frequency: {temporal}. "
                f"Must be one of {valid_temporals}"
            )

        if spatial_type not in self.SPATIAL_TYPES:
            raise ValueError(
                f"Invalid spatial type: {spatial_type}. "
                f"Must be one of {self.SPATIAL_TYPES}"
            )
    ...
    def _validate_kwargs(self, **kwargs: Any) -> None:
        ...
        if kwargs["community"] not in self.COMMUNITIES:
            raise ValueError(
                f"Invalid community: {kwargs['community']}. "
                f"Must be one of {self.COMMUNITIES}"
            )

        if kwargs["spatial_type"] not in self.SPATIAL_TYPES:
            raise ValueError(
                f"Invalid spatial_type: {kwargs['spatial_type']}. "
                f"Must be one of {self.SPATIAL_TYPES}"
            )
        ...
        if kwargs["spatial_type"] == "point":
            ...
            if "latitude" not in kwargs or "longitude" not in kwargs:
                raise ValueError(
                    "Point queries require latitude and longitude parameters"
                )
            ...
            if missing:
                raise ValueError(
                    f"Regional queries require {', '.join(required_regional)} "
    ...
    def query(self, query: str, **kwargs: Any) -> pd.DataFrame:
        ...
        try:
            ...
            raise requests.RequestException(
                f"NASA POWER API request failed: {e}"
            ) from e
    ...
    def _parse_nasa_response(
        ...
        if not records:
            ...
            return pd.DataFrame(
                columns=["parameter", "date", "value", "latitude", "longitude"]
            )
