...
def main():
    ...
    for param in common_params:
        if param in record_types:
            ...
            print(
                f"      Available in: {len(metadata['available_in'])} "
                f"community/temporal combinations"
            )
    ...
    print(
        "\n4. Querying temperature data (T2M) for Northeast US region "
        "(regional query)..."
    )
    ...
    try:
        ...
        print("   Regional query successful!")
        print(f"   Result type: {type(result)}")
        print(f"   Data shape: {result.shape}")
        print(f"   Columns: {list(result.columns)}")
        ...
        print(
            "   (This might be expected - not all parameters support regional queries)"
        )
    ...
    if "T2M" in record_types:
        ...
        if "spatial_parameters" in t2m:
            print("   Spatial parameters by type:")
